...
trait EstudianteTraitProps {
  ...
  public function getNombreCompleto($orden='a1 a2, n', $sanear=true, $mayuscula=false): string 
  {
  ...
  public function isNuevoIco(): string { return ( $this->isNuevo() ? '<span class="w3-text-red">NEW</span>' : '' ); }
  public function getPagoPension(): string { return 'Pago Pensión: '.OdaUtils::nombreMes((int)$this->mes_pagado).' de '.$this->annio_pagado; }
  public function getDebePreicfes(): string { return 'Debe Preicfes: '.(($this->is_debe_preicfes) ? 'SI' : 'NO'); }
  public function getDebeAlmuerzos(): string { return 'Debe Almuerzos: '.(($this->is_debe_almuerzos) ? 'SI' : 'NO'); }

  public function getAnnioNac() { return (($this->fecha_nac) ? date('Y', strtotime($this->fecha_nac)) : ''); }
  public function getMesNac(): string { return (($this->fecha_nac) ? date('M', strtotime($this->fecha_nac)) : ''); }
  public function getDiaNac() { return (($this->fecha_nac) ? date('d', strtotime($this->fecha_nac)) : ''); }
